...
ILOLAZYCONSTRAINTCALLBACK2(LazyLeafSubgraphCallback, graphFF, G, IloIntVarArray, x)
{
...
ILOUSERCUTCALLBACK2(UserLeafSubgraphCallback, graphFF, G, IloIntVarArray, x)
{
...
void BilevelModel_withLeaf(graphFF G, int k)
{
	IloEnv env;
	IloModel model(env);
	...
	try{
		...
		sec = cplex.use(LazyLeafSubgraphCallback(env, G, x));
		
		IloCplex::Callback cut;
		cut = cplex.use(UserLeafSubgraphCallback(env, G, x));
