...
/***************************************************************************/
{
	...
	for (int ik = 0; ik < k; ik++) {
		...
		int dummy = 0;
		for (int v = 0; v < G->n; v++) {
			if(useWeight>0)	obj[dummy] = vertexWeight[v];
			else obj[dummy] = 1.0;
			lb[dummy] = 0.0;
...
/***************************************************************************/
{
	...
	int option3 = 0;//ct11
	int option4 = 0;//ct12
	int optionClique = 1;//search k-1 cliques, the first one must be assign to V_1 or V_0, the second one must be assign to V_2, v_1 or V_0 ...
	printf("Compact\n");
		//cin.get();
	int i;
...
/***************************************************************************/
{
	...
	double weightSum=vertex_number;
	if(useWeight>0){
		weightSum=0.0;
		for(int i=0; i<vertex_number; i++)
			weightSum=weightSum+vertexWeight[i];
	}

	if(output.is_open())
	{
			output.precision(10);
			output 	<< weightSum-optValue << "\t" << weightSum-best_ub << "\t" << solution_time 
				<< "\t" << lp_stat << "\t" << numcols_C << "\t" << numrows_C  << "\t" << nodes_compact;
	}
	/////////////////////////
	free(x_exact);
	////////////////////////
