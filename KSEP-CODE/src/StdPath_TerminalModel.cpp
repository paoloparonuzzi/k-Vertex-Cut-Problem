...
ILOLAZYCONSTRAINTCALLBACK6(LazyPathCallback, graphFF, G, IloIntVarArray, x, IloIntVarArray, z, double**, weight, double**, dist, int**, pred)
{
	int predIndex, i, j;
	double weightZ;
	IloEnv env = getEnv();
...
ILOUSERCUTCALLBACK6(UserPathCallback, graphFF, G, IloIntVarArray, x, IloIntVarArray, z, double**, weight, double**, dist, int**, pred)
{
...
void StdPath_TerminalModel(graphFF G, int k)
{
	int i, j;
	...
	try{
		...
		cut = cplex.use(UserPathCallback(env, G, x, z, userWeight, userDist, userPred));		
		IloCplex::Callback sec;	
		sec = cplex.use(LazyPathCallback(env, G, x, z, lazyWeight, lazyDist, lazyPred));
